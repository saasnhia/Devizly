...
function ScrollSeparator() {
  ...
  return (
    <div className="flex justify-center py-12">
      <div
        ref={ref}
        className="h-px w-24 bg-[#5B5BD6]/30 origin-left"
...
function ScrubBlock({
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      // Title scrub
      const title = ref.current!.querySelector(".vp-title");
      if (title) {
        gsap.fromTo(
          title,
      ...
      // Proof element scale
      const proof = ref.current!.querySelector(".vp-proof");
      if (proof) {
        gsap.fromTo(
          proof,
          { scale: 0.9, opacity: 0 },
      ...
      // Subtitle
      const sub = ref.current!.querySelector(".vp-sub");
      if (sub) {
        gsap.fromTo(
          sub,
          { y: 15, opacity: 0 },
...
function SignatureSVG() {
  ...
  return (
    <svg ref={ref} viewBox="0 0 180 50" className="w-full h-10 text-slate-300">
      <path
        d="M10 35 Q40 5 70 30 T130 15 T170 28"
        stroke="currentColor"
        strokeWidth="2"
        fill="none"
        strokeLinecap="round"
        className={`sig-animate ${inView ? "drawing" : ""}`}
      />
    </svg>
  );
}
...
export function ValueProposition() {
  return (
    <section className="relative">
      {/* Block A — Describe */}
      <ScrubBlock direction="left">
        <div className="py-24 lg:py-32">
          <div className="mx-auto max-w-6xl px-6">
            <h2 className="vp-title text-4xl font-bold text-white tracking-tight lg:text-6xl">
              Décrivez votre prestation<br className="hidden sm:block" /> en une phrase.
            </h2>
            <div className="vp-proof mt-8 max-w-2xl rounded-xl border border-white/[0.08] bg-white/[0.04] p-5">
              <p className="text-slate-400">
                &ldquo;Site e-commerce pour restaurant gastronomique avec réservation en ligne&rdquo;
              </p>
            </div>
            <p className="vp-sub mt-4 text-sm text-slate-500">
      ...
      {/* Block B — Sign */}
      <ScrubBlock direction="right">
        <div className="py-24 lg:py-32">
          <div className="mx-auto max-w-6xl px-6">
            <h2 className="vp-title text-4xl font-bold text-white tracking-tight lg:text-6xl lg:text-right lg:ml-auto">
              Votre client signe<br className="hidden sm:block" /> depuis son téléphone.
            </h2>
            <div className="vp-proof mt-8 lg:ml-auto lg:mr-0 w-fit">
              {/* Phone mockup */}
      ...
      <ScrubBlock direction="left">
        <div className="py-24 lg:py-32">
          <div className="mx-auto max-w-6xl px-6">
            ...
            <div className="vp-proof mt-8 max-w-sm rounded-xl border border-white/[0.08] bg-white/[0.04] p-5">
